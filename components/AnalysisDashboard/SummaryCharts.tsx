...
const SummaryCharts: React.FC<{ data: ChartDataItem[] }> = ({ data }) =>
{
    ...
    const calculateRadius = () =>
    {
        ...
        { // mobile
            return {
                inner: 50,  // Slightly reduced
                outer: 75   // Slightly reduced
            };
        } else if (width < 768)
        { // tablet
            return {
                inner: 70,  // Slightly reduced
                outer: 95   // Slightly reduced
            };
        } else
        { // desktop
            return {
                inner: 90,  // Slightly reduced
                outer: 120  // Slightly reduced
            };
        }
    };
    ...
    return (
        <div className="grid grid-cols-1 gap-3 pb-3"> {/* Reduced gap and padding */}
            <Card className="bg-white/90 backdrop-blur-sm px-2 sm:px-4 py-3 min-h-[500px] sm:min-h-[400px] lg:min-h-[350px]"> {/* Reduced heights and padding */}
                <CardHeader className="pb-2"> {/* Reduced padding */}
                    <CardTitle>Score Distribution (Pie Chart)</CardTitle>
                </CardHeader>
                <CardContent className="pt-0"> {/* Removed top padding */}
                    <div className="h-[300px]"> {/* Reduced height */}
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
                                    activeIndex={activeIndex}
                                ...
                                <Legend
                                    ...
                                    height={32} 
                                    content={({ payload }: any) => (
                                        <div className="flex flex-wrap justify-center gap-3 mt-2"> {/* Reduced gap and margin */}
                                            {payload.map((entry: any, index: number) => (
                                                <div
                                                    key={`legend-${index}`}
                                                    className="flex items-center cursor-pointer"
